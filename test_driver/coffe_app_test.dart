// to run in terminal
// flutter drive --target=test_driver\coffe_app.dart
...
void main() {
  group('Drinks Test', () {
    ...
    test('find carousel tea card', () async{
      expect(await driver.getText(teaTypeFinder, timeout: Duration(seconds: 30)), 'Tea');
    });

    test('find black tea', () async {
      await driver.tap(teaTypeFinder);
      expect(await driver.getText(blackTeaCardFinder), 'Black Tea');
    });
    
    test('find black tea order price', () async {
      await driver.tap(blackTeaCardFinder);
      expect(await driver.getText(blackTeaOrderFinder), 'Price: \$4.12');
    });
